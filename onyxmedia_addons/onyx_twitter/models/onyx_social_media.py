...
class SocialMediaTwitter(models.Model):
    ...
    onyx_media_type = fields.Selection(selection_add=[('twitter', 'Twitter')])
    ...
    def _action_add_account_onyx(self):
        ...
        if self.onyx_media_type != 'twitter':
            return super(SocialMediaTwitter, self)._action_add_account_onyx()
        ...
        twitter_consumer_secret_key = request.env['ir.config_parameter'].sudo().get_param('onyx_twitter.consumer_secret_key')
        callback_url = url_join(self.get_base_url(), "social_twitter/callback")
        print('kdskdjfkjhdlgkj',twitter_consumer_key,twitter_consumer_secret_key)
        ...
        try:
            ...
            print(
                'Error! Failed to get request token.')        # if twitter_consumer_key and twitter_consumer_secret_key:
