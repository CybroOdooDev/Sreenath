...
class ResPartner(models.Model):
    ...
    def _compute_dr_social_account_count(self):
        res = {
            ...
            for r in self.env['social.account'].read_group(
    ...
    def _compute_dr_social_posts_count(self):
        for partner in self:
            social_accounts = self.env['social.account'].search([('dr_partner_id', '=', self.id)])
            partner.dr_social_posts_count = self.env['social.post'].search_count([('account_ids', 'in', social_accounts.ids)])
