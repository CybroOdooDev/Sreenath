...
class DeliveryCarrier(models.Model):
    ...
    def sendcloud_get_tracking_link(self, picking):
        parcels = picking.sendcloud_parcel_ids
        url_list = [(parcel.tracking_number, parcel.tracking_url) for parcel in parcels]
        return len(url_list) == 1 and url_list[0][1] or json.dumps(url_list)

    def sendcloud_cancel_shipment(self, picking):
        ctx = self.env.context.copy()
        ctx["skip_sync_picking_to_sendcloud"] = True
        ctx["skip_cancel_parcel"] = True
        deleted_parcels = []
    ...
    def _sendcloud_get_price_per_country(self, country_code):
        self.ensure_one()
        force_update = self.env.context.get("sendcloud_force_update_method")
        if force_update:
