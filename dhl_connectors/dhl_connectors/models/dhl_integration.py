...
class SendCloudIntegration(models.Model):
    ...
    parcel_international_number = fields.Char('Parcel International Account No')
    parcel_europe_number = fields.Char('Parcel Europe Account No')
    parcel_return_number = fields.Char('Parcel Return Account No')
        ...
        for order in self:
            for rec in order.picking_ids:
                ...
                if order.partner_id.country_id.intrastat:
                    ...
                    customer_account_no = dhl_cred.parcel_europe_number
                else:
                    parcel_product = 'ParcelEurope.parcelinternational'
                    customer_account_no = dhl_cred.parcel_international_number
                                                    ...
                                                    "cPAN": {
                                                        "addresses": {
                                                            "sender": {
                                                                ...
                                                                "customerAccountNr1": customer_account_no,
                                                    ...
                                                    "cPAN": {
                                                        "addresses": {
                                                            "sender": {
                                                                ...
                                                                "referenceNr": "REF45678901234567890123456789012345",
                                                                "customerIdentification": dhl_cred.customer_identification_no,
                                                                "customerAccountNr1": dhl_cred.parcel_return_number,
                                                                "hs_code": order.order_line[0].product_id.code
                                                            },
                                                            "recipient": {
                                                                "type": "parcelshops",
    ...
    def load_ccustom_items(self):
        for order in self:
            ...
            for rec in order.order_line:
                if rec.product_id:
                    if rec.product_id.intrastat_id:
                        if rec.product_id.intrastat_origin_country_id.code:
                            if rec.product_id.intrastat_origin_country_id.code:
                                    ...
                                    if rec.product_id.weight:
                                        if rec.price_subtotal:
                                            items.append({
                                                ...
                                                "customsTariffNumber": rec.product_id.intrastat_id.code,
                                            ...
                                            raise ValidationError(
                                                _("Please check wheather the total is set in the Order "))
                                    else:
                                        raise ValidationError(
                                            _("Please check wheather the weight is set for the product "))
                                else:
                                    raise ValidationError(
                                        _("Please check wheather the Quantity is given in the order"))
                            else:
                                raise ValidationError(
                                    _("Please check wheather the intrastat country code is set in the Product"))
                            return items

    def load_ccustom_payload(self, cpan_no):
        for order in self:
            for rec in order.picking_ids:
                dhl_cred = self.env['dhl.integration'].search([], limit=1)
                ...
                if dhl_cred:
                    ...
                    payload = {
                        "dataElement": {
                            "version": "0200",
                            "parcelOriginOrganization": order.partner_id.country_id.code,
                            "parcelDestinationOrganization":  dhl_cred.return_address.country_id.code,
                            "general": {
                                "parcelIdentifier": cpan_no,
            ...
            for rec in order.picking_ids:
                if rec != 0:
                    if rec.delivery_type == 'dhl':
                        ...
                        querystring_pdf_false = {"generateLabel": "false"}
                        ...
                        cpan_responses_ccustom = requests.request("POST", cpan_url, json=cpan_payload, headers=headerss,
                                                              params=querystring_pdf_false)


                        cpan_no = cpan_payload['dataElement']['general']['parcelIdentifier']

                        ccustoms_payload = order.load_ccustom_payload(cpan_no)

                        ccustom_responses_pdf = requests.request("POST", ccustom_url, json=ccustoms_payload,
                                                                 headers=headerss)
                        ...
                        if cpan_responses_pdf.status_code == 200:
                            ...
                            pick.dhl_ids.tracking_number = cpan_responses_pdf.headers['shipmentId']
                            language = self.env['res.lang'].search([('code', '=', order.partner_id.lang)])
                            lang_code = language.code.replace("_", "-").lower()
                            pick.dhl_ids.tracking_url = "https://www.dhl.com/" + lang_code + "/home/tracking/tracking-global-forwarding.html?submit=1&tracking-id=" + str(pick.dhl_ids.tracking_number)
                        ...
                        if ccustom_responses_pdf.status_code == 200:
                            custom_sequence_id = self.env['ir.sequence'].next_by_code('dhl.integration.custom')

                            ccustom_reports = self.env['ir.attachment'].sudo().create({
                                 'name': custom_sequence_id + '.pdf',
                                'type': 'binary',
                                'datas': base64.encodestring(ccustom_responses_pdf.content),
                                'mimetype': 'application/x-pdf',
                                'res_model': 'dhl.parcel',
                                'res_id': parcel.id,
                            })
                            pick.dhl_ids.ccustom_pdf = ccustom_reports.ids
                        # else:
                        #     raise ValidationError(
                        #         _(str(ccustom_responses_pdf.statusMessage)))
