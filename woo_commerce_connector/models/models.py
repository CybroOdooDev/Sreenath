...
class StockPickingInherit(models.Model):
    ...
    delivery_time_slot = fields.Char("Delivery Time Slot", compute="_compute_delivery_time_slot")
    
    def _compute_delivery_time_slot(self):
        so = self.env['sale.order'].search([('name', '=', self.origin)])
        self.delivery_time_slot = so.del_time_slot
        ...
        for rec in self:
            if rec.instance_id and rec.woo_id and not self._context.get('webhook'):
                ...
                if 'veg' in vals_list.keys() and veg_woo_id:
                    ...
                    attr_flag = True
                    if vals_list['veg']:
                        if vals_list['veg'] == 'yes':
                            veg = 'yes'
                        elif vals_list['veg'] == 'no':
                            veg = 'No'
                        else:
                            veg = " "
...
class ProductProductInherited(models.Model):
    ...
    def create(self, vals_list):
        """
            For creating variation in woocommerce.
        """
...
class SaleOrderInherit(models.Model):
    ...
    del_time_slot = fields.Char("Delivery Time Slot")
    ...
    @api.onchange('del_time_slot')
    def _onchange_del_time_slot(self):
        do = self.env['stock.picking'].search([('origin', '=', self.name)])
        for rec in do:
            _logger.info("del-order.... %s", rec)
            rec.delivery_time_slot = self.del_time_slot
            _logger.info("del-time-slot.... %s", rec.delivery_time_slot)
