...
class WooWizard(models.TransientModel):
    ...
    def get_woo_import(self):
        ...
        if self.product_check:
            ...
            wcapi = self.get_api()
            prods = wcapi.get('products', params={'per_page': 100}).json()
            ...
            for recd in orders:
                if str(recd.get('id')) not in woo_ids and recd.get('status') != 'cancelled':
                    ...
                    for line in recd.get('line_items'):
                        ...
                        price_unit = round(float(line.get('price')) * currency_rates[self.currency], 2)
                        vals_line = (0, 0, {
                            'product_id': product_id.id,
                            'name': product_id.description if product_id.description else product_id.name,
                            'product_uom_qty': line.get('quantity'),
                            'price_unit': price_unit + float(line.get('total_tax')) / line.get('quantity'),
                        })
                    ...
                    if recd.get('shipping_lines'):
                        for line in recd.get('shipping_lines'):
                            ...
                            if float(line.get('total')) > 0:
                                _logger.info("service id...... %s", service_id)
                                for rec in service_id:
                                    vals_line = (0, 0, {
                                        'product_id': rec.id,
        ...
        if self.product_check:
            ...
            for recd in products:
                ...
                if recd.veg and veg_woo_id:
                    ...
                    if recd.veg == 'yes':
                        veg_data['options'] = ['yes']
                    elif recd.veg == 'no':
                        veg_data['options'] = ['No']

                    att_id.append(veg_data)
