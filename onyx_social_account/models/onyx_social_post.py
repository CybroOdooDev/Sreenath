...
class SocialPost(models.Model):
    ...
    def action_post(self):
        ...
        #     print('acc_idacc_id', acc_id)
        #     print('onyx_media_type', acc_id.onyx_media_type)
        #     print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', acc_id.read())
        #     if acc_id.onyx_media_type == 'facebook':
        #         params = {
        ...
        for acc_id in self.account_ids:
            print('acc_idacc_id', acc_id)
            account_id = self.env['onyx.social.account'].search([]).filtered(lambda x: x.id == acc_id.id)
            print('account_id', account_id)

            for rec in account_id:
                print('rec.onyx_media_type', rec.onyx_media_type)
                media_type.append(rec.onyx_media_type)
    ...
    def twitter_post(self):
        ...
        img = self.env['ir.config_parameter'].sudo().set_param('onyx_twitter.twitter_image',
                                                               url_join(self.get_base_url(),
                                                                        str(self.image_ids.image_src)))
        api = tweepy.API(auth)
        try:
    ...
    def _get_facebook_redirect_uri(self):
        ...
        print('_get_facebook_redirect_uri_get_facebook_redirect_uri_get_facebook_redirect_uri', self.id)
        return request.redirect('/social_facebook_post/' + str(self.id) + '?close')
