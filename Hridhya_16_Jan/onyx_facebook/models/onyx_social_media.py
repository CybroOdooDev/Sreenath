...
class SocialFacebookOnyx(models.Model):
    ...
    def _action_add_account_onyx(self):
        ...
        with open('/home/cybrosys/PycharmProjects/odoo15E/odoo15/onyxmedia_latest/Hridhya_16_Jan/dr_social_website/static/onyxm.key') as f:
            privateKey = f.read()
        print('***********', self.env.user.read())
        print('***********', self.env.user.name)
        print('!!!!!!!!!!', self.env.user.profile_key)
        payload = {'domain': self.env.user.name,
                   'privateKey': self.env.user.profile_key,
    ...
    #     print('**********************************', url_join(self.get_base_url(), 'social_linkedins/callback'))
    #     return url_join(self.get_base_url(), '/social_facebook_post')
