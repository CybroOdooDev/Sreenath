...
class OnyxInstagram(models.Model):
    ...
    onyx_media_type = fields.Selection(selection_add=[('instagram', 'Instagram')])

    def _action_add_account_onyx(self):
        self.ensure_one()

        if self.onyx_media_type != 'instagram':
            return super(OnyxInstagram, self)._action_add_account_onyx()
        # model = self.env['onyx.instagram'].sudo().search([], limit=1)
        cliend_id = request.env['ir.config_parameter'].sudo().get_param('onyx_instagram.client_id_instagram')
        params = {
            'client_id': cliend_id,
            'redirect_uri': 'http://localhost:8019/social_instagram',
